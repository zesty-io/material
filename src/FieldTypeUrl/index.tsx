...
export interface FieldTypeUrlProps extends Omit<
  ...
  return (
    <FormControl fullWidth required={required}>
      ...
      >
        ...
        <Box sx={{ order: 3, flex: 1, textAlign: "right" }}>
      ...
      <MuiTextField
        ...
        error={
          (value && !inputRef?.validity.valid) || value?.length > maxLength
        }
        helperText={
          value?.length > maxLength
            ? "Your input is over the specified limit"
            : value && !inputRef?.validity.valid
              ? "Your input is not a valid url"
              : helperText
        }
